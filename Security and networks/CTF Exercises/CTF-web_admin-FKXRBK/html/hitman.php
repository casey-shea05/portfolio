<?php
  ...
  if($_REQUEST['name']) {
    ...
    if($_REQUEST['areyousure'] == 'on'
      ...
      // kill them.
      $sql = "UPDATE users SET killed_on = DATE('now'), killed_by = '" .
      	$_SESSION['user']['id'] . "' WHERE name = '" . SQLite3::escapeString($_REQUEST['name']) . "';";
      $website_db->query($sql) and $done = true;
    }
  }

  // get a list of unkilled people
  $sql = "SELECT name, full FROM users WHERE killed_on IS NULL ORDER BY full;";
  $result = $website_db->query($sql); //or die(mysqli_error($db));
