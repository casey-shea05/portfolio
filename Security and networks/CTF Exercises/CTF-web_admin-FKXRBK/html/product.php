<?php
  ...
  if(isset($_REQUEST['quantity']) && is_numeric($_REQUEST['quantity'])) {
    ...
    $sql = "insert into basket (user_id, product_id, quantity) values('" . 
      SQLite3::escapeString($_SESSION['user']['id']) . "', '" .
      SQLite3::escapeString($_GET['id']) . "', " .
      SQLite3::escapeString($_REQUEST['quantity']) . ") on conflict(user_id,product_id) do update set quantity = quantity + " .
      SQLite3::escapeString($_REQUEST['quantity']) . ";";
  ...
  if(isset($_GET['id'])) {
    ...
    $sql .= " AND p.id=".SQLite3::escapeString($_GET['id']);
  // are we searching?
