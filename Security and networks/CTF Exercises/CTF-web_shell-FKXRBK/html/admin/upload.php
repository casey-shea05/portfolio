<?php
// TODO: the hardcoded relative pathnames will need changing on real server
session_start();
...
if($_POST) {
  ...
  if($_SESSION["csrf"] == $_POST["csrf"]) {
   $target = __DIR__."/../img/uploads/" . basename($_POST["filename"]);
    ...
    if(@pathinfo($_FILES["file"]["name"], PATHINFO_EXTENSION)
        ...
        if(move_uploaded_file(basename($_POST["filename"]), $_FILES["file"]["tmp_name"])) {
        $danger = $drugsarebad ? 0 : ((is_numeric($_POST["danger"]) and in_array($_POST["danger"], array(0, 1))) ? $_POST["danger"] : 0);
        require_once("../sql.php");
  
  	  	$sql = "INSERT INTO products (name, description, image, danger, price) VALUES('".SQLite3::escapeString(htmlentities($_POST["name"]))."', '".
          SQLite3::escapeString( htmlentities($_POST["desc"]))."', '".
          SQLite3::escapeString( "/../img/uploads/" . basename($_POST["filename"]))."', '".
          SQLite3::escapeString( $danger)."', '".
          SQLite3::escapeString( $_POST['price'])."');";
  
        $result = $website_db->query($sql);
        if(!$result) {
          $errors[] = $website_db->lastErrorMsg;
          unlink($target);
        } else {
          $done = true;
        }
...
$file = basename(@tempnam(__DIR__."/../img/uploads", strftime("%Y-%m-%d_", time())));
$file .= ".jpg";
