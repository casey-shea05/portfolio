<?php
  ...
  if(isset($_POST['empty']) && $_POST['empty'] == true) {
    // empty the basket (uses a stored procedure) - doesnt work :(
  ...
  if(isset($_POST['buy']) && $_POST['buy'] == true) {
    ...
    if($noexp === false && $nocc === false && $nocvv === false) {
      ...
      // BUG: 'buy' function is not implemented, need to port to sqlite somehow or just write the query manually
      $sql = sprintf("CALL buy(%d, '%s', '%s', str_to_date('%s-%s', '%%Y-%%m-%%d'));",
          $_SESSION['user']['id'],
           SQLite3::escapeString($_POST['cc']),
           SQLite3::escapeString($_POST['cvv']),
           SQLite3::escapeString($_POST['expire']),
	  '01'
        );
      echo($sql);
      $result = $website_db->query($sql);
