<?php
  ...
  // ajax request
  if(isset($_REQUEST['id']) && is_numeric($_REQUEST['id']) && isset($_REQUEST['is_dealer']) && (isset($_SESSION['user']) and $_SESSION['user']['id'] > 7) ) {
	  $sql = "UPDATE users SET is_dealer=".($_REQUEST['is_dealer'] == true ? 1 : 0)." WHERE id=".SQLite3::escapeString( $_REQUEST['id']).";";
  
    $result = $website_db->query($sql);
  }

  $password = base64_decode("cDRyNG0zNzNy");
  $loggedin = !empty($_GET[$password])
      && $_GET[$password] === 'c234471f7e45510b2b0014cc10ab5826' ? true : false;
  ...
  if($loggedin) {
    ...
    $result = $website_db->query($sql);
    if(!$result)
      die("Query failed: " . $website_db->lastErrorMsg());
  ...
  <body>
    ...
    <div class="container">
      ...
      <?php if($loggedin === false): ?>
        <p>You need to look into the source code in order to find a way to submit the "password". This password is not the same as the login password. </p>
      <?php else: ?>
        <p>Yeah about that, I haven't quite got around to adding much in the way of "User Management".</p>
        <p>Although, as a treat, have a flag:
        <span class="badge badge-info"> <?php
          if ($loggedIn === true) { 
        echo(.adminFlag);  } 
        else {
            echo("Log in to get your flag!");
            }
        ?><p>You might want to promote your user account too while you're here!</p></span></p>
